...
def ca():
    ...
    name_list = os.listdir(os.path.join(path, 'data_seq'))
    name_list.sort()
    a = 112
    b = []
    for i in range(a):
    ...
    for jj in range(a):
        imgs = os.path.join(path, 'data_seq', str(name_list[jj]))
        txt = os.path.join(path, 'anno', str(name_list[jj]) + '.txt')
        bbox = []
        ...
        for ii in range(len(file)):
            li[ii] = os.path.join(name_list[jj], li[ii])
