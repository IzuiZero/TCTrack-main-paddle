...
class Augmentation:
    def __init__(self, shift, scale, blur, flip, color):
        ...
        self.flip = flip
        self.color = color
        self.rgbVar = paddle.to_tensor(
            [[-0.55919361,  0.98062831, - 0.41940627],
             [1.72091413,  0.19879334, - 1.82968581],
             [4.64467907,  4.73710203, 4.88324118]], dtype='float32')
    ...
    def _crop_roi(self, image, bbox, out_sz, padding=(0, 0, 0)):
        ...
        c = -a * bbox[0]
        d = -b * bbox[1]
        mapping = paddle.to_tensor([[a, 0, c],
                                    [0, b, d]], dtype='float32')
        crop = cv2.warpAffine(image, mapping.numpy(), (out_sz, out_sz),
    ...
    def _color_aug(self, image):
        offset = paddle.matmul(self.rgbVar, paddle.randn([3, 1]))
        offset = paddle.flip(offset, [0])  # bgr 2 rgb
        offset = offset.flatten()
        image = image - offset.numpy()
        return image
