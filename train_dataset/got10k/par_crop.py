...
def crop_video(video, d_set, crop_path, instanc_size):
    if video != 'list.txt':
        video_crop_base_path = os.path.join(crop_path, video)
        if not os.path.isdir(video_crop_base_path): os.makedirs(video_crop_base_path)
        gt_path = os.path.join(dataset_path, d_set, video, 'groundtruth.txt')
        images_path = os.path.join(dataset_path, d_set, video)
        ...
        for idx, gt_line in enumerate(groundtruth):
            ...
            cv2.imwrite(os.path.join(video_crop_base_path, '{:06d}.{:02d}.z.jpg'.format(int(idx), int(0))), z)
            cv2.imwrite(os.path.join(video_crop_base_path, '{:06d}.{:02d}.x.jpg'.format(int(idx), int(0))), x)

def main(instanc_size=511, num_threads=24):
    crop_path = './crop{:d}'.format(instanc_size)

    if not os.path.isdir(crop_path): os.mkdir(crop_path)
    for d_set in sub_sets:
        save_path = os.path.join(crop_path, d_set)
        videos = os.listdir(os.path.join(dataset_path,d_set))
